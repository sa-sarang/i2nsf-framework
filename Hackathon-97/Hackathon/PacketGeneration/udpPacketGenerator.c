...
int
generateUdpPacket (char* interface, char* srcIPv4Address, char* destIPv4Address, int srcPort, int destPort, char* contents)
{
  //printf("request: interface: %s from %s:%d to %s %d\n", interface, srcIPv4Address, srcPort, destIPv4Address, destPort);
  ...
  if (ioctl (sd, SIOCGIFINDEX, &ifr) < 0) {
    ...
    printf("ioctl() failed to find interface \n");
    return (EXIT_FAILURE);
  }
  close (sd);
  ...
  strcpy (src_ip, srcIPv4Address);

  // Destination URL or IPv4 address: you need to fill this out
  strcpy (target, destIPv4Address);
