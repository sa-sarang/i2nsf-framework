...
static bool add_policy(struct policy *policy_container) {
    ...
    if(MysqlInsertQuery("`dpi_policy`", columns, values)) {
        printf("\n\n New Policy Created \n\n");
		printf("Test2\n");
        //strcpy(columns, "`event`, `sip_uri`, `user_agent`, `action`, `policy_id`");
		
		printf("URI NUM : %d \n", policy_container->uri_num);
		for(i = 0; i < policy_container->uri_num; i++) {
			printf("URI Result: %d, %s, %d, %d\n",
				policy_container->event,
				policy_container->sip_uri[i],
				policy_container->action,
				policy_container->rule_id);

			strcpy(columns, "`event`, `sip_uri`, `action`, `policy_id`");
			n = sprintf(values, "%d, \"%s\", %d, %d",
								policy_container->event,
			                    policy_container->sip_uri[i],
                                policy_container->action,
                                policy_container->rule_id);
			values[n] = '\0';
            if(!MysqlInsertQuery("`dpi_rule`", columns, values)) {
				char where[20];
	            n = sprintf(where, "`policy_id`=%d", policy_container->rule_id);
		        where[n] = '\0';
			    MysqlDeleteQuery("`dpi_rule`", where);
		...
		policy_container->uri_num = 0;
		printf("USER AGENT NUM : %d \n", policy_container->user_agent_num);
		for(i = 0; i < policy_container->user_agent_num; i++) {
			printf("USER AGENT Result: %d, %s, %d, %d\n",
				policy_container->event,
				policy_container->sip_user_agent[i],
				policy_container->action,
				policy_container->rule_id);

			strcpy(columns, "`event`, `sip_user_agent`, `action`, `policy_id`");
			n = sprintf(values, "%d, \"%s\", %d, %d",
								policy_container->event,
                                policy_container->sip_user_agent[i],
                                policy_container->action,
		...
		policy_container->user_agent_num = 0;
...
static bool is_policy_exists(char *policy_name) {
    ...
    sqlResult = MysqlSelectQuery("`dpi_policy`", "`policy_name`", where, true);
...
static int tr_commit(struct confd_trans_ctx *tctx) // use the xml files.
{
    ...
    confd_value_t *uri_list, *user_agent_list;
    char where[100] = {0};
    int i;
        ...
        if (strcmp(item->callpoint, "hcp") == 0) {
            ...
            case C_SET_ELEM:
                ...
                case dpi_rule_id:
                    ...
                    break;
				case dpi_called_voip:
						if(CONFD_GET_BOOL(item->val))
                        policy_container->event = VOIP;
					break;
				case dpi_called_volte:
						if(CONFD_GET_BOOL(item->val))
                        policy_container->event = VOLTE;
					break;
                case dpi_sip_uri:
					uri_list = CONFD_GET_LIST(item->val);
					policy_container->uri_num = CONFD_GET_LISTSIZE(item->val);

					for(i = 0; i < policy_container->uri_num; i++) {
						policy_container->sip_uri[i] = (char *) CONFD_GET_BUFPTR(&uri_list[i]);
					}
					
	//				policy_container->sip_uri = atoi((char *) CONFD_GET_BUFPTR(item->val));	 
                    break;
                case dpi_sip_user_agent:
					user_agent_list = CONFD_GET_LIST(item->val);
					policy_container->user_agent_num = CONFD_GET_LISTSIZE(item->val);

					for(i = 0; i < policy_container->user_agent_num; i++) {
						policy_container->sip_user_agent[i] = (char *) CONFD_GET_BUFPTR(&user_agent_list[i]);
					}
//					policy_container->sip_user_agent = atoi((char *) CONFD_GET_BUFPTR(item->val));	
                    break;
                case dpi_permit:
                    if(CONFD_GET_BOOL(item->val))
            ...
            case C_REMOVE:
                ...
                if(!MysqlDeleteQuery("`dpi_policy`", where)) {
...
static int policy_num_instances(struct confd_trans_ctx *tctx, confd_hkeypath_t *keypath) {
    ...
    MYSQL_RES *sqlResult = MysqlSelectQuery("`dpi_policy`", "COUNT(*)", "1=1", false);
    MYSQL_ROW row = MysqlGetRow(sqlResult);
...
//    confd_init("firewall_daemon", stderr, debuglevel);
	confd_init("dpi_daemon", stderr, debuglevel);

    /* Initialize daemon context */
    if ((dctx = confd_init_daemon("dpi_daemon")) == NULL)
